...
export default async function ReviewPage({
  ...
  // Server-only state: appointment doesn't exist. Everything else (already
  // reviewed, not eligible, freshly submitted) is handled inside ReviewForm,
  // so the client-side "just submitted" thanks state survives the auto-refresh
  // that fires after the server action.
  if (!appointment) {
    return (
      <Shell>
  ...
  const serviceName =
    loc === "ar" ? appointment.service.nameAr : appointment.service.nameEn;
  ...
  const hasExistingReview = Boolean(appointment.review);
  const isEligible = appointment.status === "COMPLETED";
  const showSessionCard = isEligible && !hasExistingReview;
